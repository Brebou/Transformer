...
def prepare_data():
    ...
    for idx, row in df.iterrows():
        tokens_fr = row['tokens_fr']
        tokens_en = row['tokens_en']
        for j in range(len(tokens_en)):
             l.append((tokens_fr, tokens_en[:j+1]))
...
class VocabDataset(TensorDataset):
    def __init__(self, 
        ...
        self.max_len_fr = max(len(x[0]) for x in l)
        self.max_len_en = max(len(x[1]) for x in l)

    def __len__(self):
        return len(self.l)
    
    def __getitem__(self, idx):
        tokens_fr = self.l[idx][0]
        tokens_en = self.l[idx][1]
